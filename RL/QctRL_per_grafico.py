...
if __name__ == "__main__":
    ...
    final_rewards_list=[]  #append final rewards for each protocol length t

    for t in np.arange(0,4.1,0.25):

        t_max = t #QUESTO VARIA!

        rewards_list = [] #append rewards for given protocol time (to be appended at fianl_rewards_list)

        for repetition in tqdm(range(10)):
        ...
        print("Time:",t)
        print("Mean final reward:", np.array(rewards_list).mean())
        print("Std final rewards:", np.array(rewards_list).std())
        print("\n")
        print("\n")

        final_rewards_list.append(rewards_list)
           
        '''fname = 'protocol'+str(t_max)+'-'+str(dt)+'.gif' #la crea solo dell'ultimo best protocol trovato
        fname = out_dir / fname
        create_gif(best_path, qstart, qtarget, fname)'''

    outname = 'fidelity_RL.txt'
    outname = out_dir / outname
    np.savetxt(outname, final_rewards_list, delimiter = ',',header="Matrix with as entries the values of fidelity dimension times x iterations")
...
times = np.arange(0,4.1,0.25)
loaded_fidelity = pd.read_csv(outname, skiprows=1,header=None)
...
ax.hlines(1,0,4, color="b", linestyle="-.")
ax.vlines(0.5,0,1,  color="b", linestyle="-.")
ax.vlines(2.5,0,1,  color="b", linestyle="-.")
ax.set_title(r"Phase Diagram"+"\n"+"Iterations for Each Point = "+str(10), fontsize=18)
ax.set_xlabel("T [a.u.]", fontsize=18)
ax.set_ylabel(r" $Fidelity$", fontsize=18)
ax.tick_params(axis='both', which='major', labelsize=14)
ax.legend(fontsize=13, loc=5)
fig.tight_layout()
fname = out_dir / "phase_diagram.pdf"
fig.savefig(fname)
plt.show()
# %%
